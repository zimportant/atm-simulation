import { faFileAlt, faVoicemail, faFileVideo, faVideo } from '@fortawesome/free-solid-svg-icons';
...
const SecurityContainer = styled(Box)`
  margin: 0 0 0px 0;
  display: flex;
  flex-direction: row;
...
const SecurityItem: React.FC<SecurityItemProps> = ({
  ...
  const itemDescription = (type: string, _content: string): any => {
    ...
    if (type === 'cctv') {
      return <ReactPlayer width="100%" url={_content} />;
    }
    if (type === 'text') {
      return (
      ...
      <Grid container item direction="column">
        ...
        <Grid container item xs={11} direction="row">
          ...
          <Grid container item justify="flex-start" direction="column">
            <Box flexDirection="row" style={{ marginTop: '12px' }}>
              {itemDescription(messageType, content)}
            </Box>
          </Grid>
        </Grid>
      </Grid>
